#!/usr/bin/env bash
## With Retry: Run NuttX CI with Docker:
...
echo Now running https://github.com/lupyuen/nuttx-release/blob/main/run-ci-retry.sh $1
set -x  ## Echo commands
...
function run_job {
  ...
  script $log_file \
    $script_option \
    "$script_dir/run-job-retry.sh $job $instance"
  popd
}
