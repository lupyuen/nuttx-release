...
##   cd $HOME/nuttx-release && sudo sh -c '. ../github-token.sh && ./run-ci-special.sh '$TMUX_PANE
##   cd $HOME/nuttx-release && sudo sh -c '. ../gitlab-token.sh && ./run-ci-special.sh '$TMUX_PANE
...
echo Now running https://github.com/lupyuen/nuttx-release/blob/main/run-ci-special.sh $1
set -x  ## Echo commands
...
function run_job {
  ...
  script $log_file \
    $script_option \
    "$script_dir/run-job-special.sh $job $instance"
  popd
}
...
function upload_log {
  ...
  local nuttx_hash=$2
  local apps_hash=$3
  local desc="[$job] Special CI Log for nuttx @ $nuttx_hash / nuttx-apps @ $apps_hash"
